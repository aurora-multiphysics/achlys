...
#include "ExtrinsicTransientTrappingMaterial.h"
#include <cstdio>

registerMooseObject("achlysApp", ExtrinsicTransientTrappingMaterial);

InputParameters ExtrinsicTransientTrappingMaterial::validParams()
{
...
//   params.addRequiredParam<Real>("n3", "possible trapping sites");
  params.addRequiredParam<Real>("n3a_actual", "modelled attainted trap density - \"a\" type");
  params.addRequiredParam<Real>("n3b_actual", "modelled attainted trap density - \"a\" type");
...
ExtrinsicTransientTrappingMaterial::ExtrinsicTransientTrappingMaterial(const InputParameters & parameters)
  : ADMaterial(parameters),
    ...
    _n3a(getParam<Real>("n3a_actual")),
    _n3b(getParam<Real>("n3b_actual")),
...
void
ExtrinsicTransientTrappingMaterial::computeQpProperties()
{


//Real T = (*_simulation_temperature[0])[_qp];
...
if (_n3a > 0.0){
  if(_q_point[_qp](0)< 0.9 *_xp){
    Real offset = _n3b;
    Real scale = _n3a - _n3b;
    Real x = _q_point[_qp](0);
    Real exp = std::exp(-1.0 * _sigmoid_sf *(x-(_n3ab_x * 1e-9)));
