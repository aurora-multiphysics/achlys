...
#include "DirichletPfcFunctionFluxRampBC.h"
#include "Function.h"

registerMooseObject("achlysApp", DirichletPfcFunctionFluxRampBC);

InputParameters
DirichletPfcFunctionFluxRampBC::validParams()
{
  InputParameters params = ADIntegratedBC::validParams();
  params.addRequiredCoupledVar("T", "Temperature");
  params.addParam<Real>("threshold", 0.0, "flux value at which desorption is explicitly calculated");
...
DirichletPfcFunctionFluxRampBC::DirichletPfcFunctionFluxRampBC(const InputParameters & parameters)
...
ADReal
DirichletPfcFunctionFluxRampBC::computeQpResidual()
{
  Real K = _K0 * std::exp((-1.0 * _E) / (8.617333E-5 * _T[_qp]) );
  Real flux_value = _flux_function.value(_t, _q_point[_qp]);
  if (flux_value > _threshold){
    Real ss_surface_conc = std::sqrt( flux_value / K) / _scale_factor;
    return _u[_qp] - ss_surface_conc;
  } else {
