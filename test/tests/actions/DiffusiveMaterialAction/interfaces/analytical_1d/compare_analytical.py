...
def generate_graphs(inputs, filename=None, show=True):
    ...
    for file, ax in zip(inputs, axes):
        ...
        X, m1_y = f1.get_values_as_np(timestep = 1, array_id=1, verbose=False)
        _, m2_y = f1.get_values_as_np(timestep = 1, array_id=2, verbose=False)
        x = X[:,0]
...
if  __name__ == '__main__':
    ...
    generate_graphs(inputs, filename='action.png')
    #inputs = {
